...
TARGET_COLUMNS = ["ts_code", "trade_date", "adj_factor"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="从 stock_daily.parquet 补齐 qteasy 的 stock_adj_factor")
    ...
    parser.add_argument("--chunk_size", type=int, default=20, help="按交易日分块写入，每块交易日数量")
    parser.add_argument("--merge_type", type=str, default="update", choices=["update", "ignore"], help="主键冲突处理")
    parser.add_argument("--dry_run", action="store_true", help="仅预览，不写入")
    return parser.parse_args()


def _read_source(path: str) -> pd.DataFrame:
    # 优先只读 adj_factor 列，降低内存占用
    try:
        df = pd.read_parquet(path, columns=["adj_factor"]).reset_index()
    except Exception:
    ...
    if not {"ts_code", "trade_date", "adj_factor"}.issubset(df.columns):
        miss = [c for c in TARGET_COLUMNS if c not in df.columns]
        raise ValueError(f"源数据缺少必要列: {miss}")
    ...
    out = out.dropna(subset=["ts_code", "trade_date", "adj_factor"]).sort_values(["trade_date", "ts_code"]).reset_index(drop=True)
    return out
...
def main() -> None:
    ...
    for idx, date_chunk in enumerate(_iter_date_chunks(udates, args.chunk_size), start=1):
        ...
        rows = ds.update_table_data("stock_adj_factor", part, merge_type=args.merge_type)
        total_rows += rows
        print(
